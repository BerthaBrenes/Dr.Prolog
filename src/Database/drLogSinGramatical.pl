...
drLogStart(SalAux,SalDoc):-atom_string(Sal,SalAux),saludo(Sal),saludoDoctor(SalDoc).
drLogStart(SalAux,SalDoc):-atom_string(Sal,SalAux),not(saludo(Sal)),noEntiendo(SalDoc).
